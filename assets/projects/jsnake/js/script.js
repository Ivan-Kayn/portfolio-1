//creating field 100x100
let field = document.createElement('div')
document.body.appendChild(field)
field.classList.add('field')
//creating 100x100 cells on field
for (let i = 1; i < 101; i++) {
    let excel = document.createElement('div')
    field.appendChild(excel)
    excel.classList.add('excel')
}
//creating positions x and y for any cell
...
let coordinates = generateSnake(),
    snakeBody = [
        document.querySelector('[posX = "' + coordinates[0] +
            '"][posY = "' + coordinates[1] + '"]'),
        document.querySelector('[posX = "' + (coordinates[0] - 1) +
            '"][posY = "' + coordinates[1] + '"]'),
        document.querySelector('[posX = "' + (coordinates[0] - 2) +
            '"][posY = "' + coordinates[1] + '"]'),
    ]
...
function move() {
    ...
    if (snakeBody[0].classList.contains('snakeBody')) {
        setTimeout(() => {
            alert('game over, you done ' + score + 'points.')
        }, 200)
        clearInterval(interval)
        snakeBody[0].style.backgroundColor = '#fff'
    }
